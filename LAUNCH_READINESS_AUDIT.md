...
at), including M9's unbounded-query remainder. No open code-level items remain.
...
- [x] **M9 — Several unbounded full-collection reads in Cloud Functions will degrade at scale.** *(fixed, including the remainder previously flagged for a product call: `calculateTrending` now queries `feed` with `orderByChild('timestamp').startAt(now - 7d)` instead of reading the whole node (the redundant in-JS week-staleness check was removed since the query now bounds it). `calculateCreatorAnalytics` now queries `users` with `orderByChild('lastActive').startAt(cutoff)` (30-day cutoff), skipping accounts that haven't been active recently instead of scanning every user ever created. `adminGetDashboard` no longer downloads the entire `users`/`feed`/`reports`/`redemptions` nodes: `totalUsers`/`totalPosts` now come from maintained counters (`stats/totalUsers`/`stats/totalPosts`, kept accurate by four new `onCreate`/`onDelete` triggers — `onUserCreatedCounter`, `onUserDeletedCounter`, `onPostCreatedCounter`, `onPostDeletedCounter` — using `.transaction()` for atomicity, same pattern as H7's view counters); the "recent" lists use `orderByChild('createdAt').limitToLast(100)`; `pendingReports`/`pendingRedemptions` use an indexed `orderByChild('status').equalTo('pending')` count instead of downloading everything and filtering in JS. Confirmed safe to drop the old `|| !r.status` "missing status" fallback: `reports`/`redemptions` are server-write-only (no client write path — `reports` is `.write: false` at the rules level, `redemptions` had no rules entry at all, i.e. root default-deny) and every creation site (`reportUser`, the auto-low-rating flag in `rateVideoImpl`, and the redeem flow) always sets an explicit `status: 'pending'`, so there's no legacy "missing status" record to account for. Added `createdAt`/`lastActive` to `users`' `.indexOn` (was `["username"]` only) and a new `redemptions` rules entry (`.indexOn: ["createdAt","status","byUid"]`, `.write: false`, admin-only `.read` — it had no rules entry before, so this is a pure index+read-scope addition, not a behavior change) for these queries to run efficiently instead of falling back to a full scan. One-time admin-CLI backfill wrote the accurate baseline (`totalUsers: 12`, `totalPosts: 8`) so the new counters didn't start from 0. Verified live: the two new counter triggers correctly increment/decrement on a real create/delete (tested against a throwaway `users/{uid}` node); each of the three bounded queries was independently re-run via the admin CLI (`firebase database:get --order-by ... --start-at/--equal-to/--limit-to-last`) and returns results matching the old unbounded logic's semantics exactly, at the current data scale. `searchUsers`/`searchVideos`/`searchHashtags` were already fixed in an earlier pass — auth check + abuse-score rate limiting added; confirmed unreachable from the live client, which uses its own bounded `limitToLast(200)` query instead, so no further action needed there.)*
